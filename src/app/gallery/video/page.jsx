...
export default function VideoGallery() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 selection:bg-emerald-500 selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="relative rounded-2xl overflow-hidden bg-slate-800/50 border border-slate-700/80 shadow-2xl p-4 sm:p-6 lg:p-8 backdrop-blur-sm">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
              ...
              {/* Contained Spotlight Box with Explicit Centered Play Button */}
              <div className="lg:col-span-7 w-full">
                <div 
                  onClick={() => openModal(featuredVideo)}
                  className="group relative w-full aspect-video rounded-xl overflow-hidden bg-black shadow-2xl border border-slate-700/60 cursor-pointer"
                >
                  ...
                  <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-emerald-500/90 text-slate-950 flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-300">
                      <Play className="w-8 h-8 sm:w-10 sm:h-10 fill-slate-950 ml-1" />
                    </div>
                  </div>

                  <div className="absolute top-3 left-3 z-20 bg-slate-900/80 backdrop-blur-md text-emerald-400 text-[10px] sm:text-xs font-semibold tracking-wide uppercase px-2.5 py-1 rounded border border-emerald-500/20 pointer-events-none">
                    Official Spotlight
                  </div>
                </div>
              </div>
      ...
      {/* Fullscreen Video Modal Player (Fixed for landscape overlays) */}
      {activeVideoModal && (
        <div 
          ref={modalContainerRef}
          className="fixed inset-0 z-50 bg-black flex flex-col justify-between animate-in fade-in duration-200"
          role="dialog"
          aria-modal="true"
        >
          {/* Top Bar - In landscape mode, hides clutter and leaves only a floating close button */}
          <div className="landscape:absolute landscape:top-3 landscape:right-3 landscape:bg-transparent landscape:border-0 p-3 sm:p-4 flex items-center justify-between border-b border-slate-800 bg-slate-950/90 z-30 shrink-0">
            ...
            <button 
              onClick={handleRequestClose}
              aria-label="Close video"
              className="p-2 sm:p-2.5 rounded-full bg-red-600/90 hover:bg-red-700 active:bg-red-800 text-white shadow-xl transition-transform active:scale-95 shrink-0 flex items-center justify-center cursor-pointer backdrop-blur-md"
            >
              <X className="w-5 h-5 sm:w-6 sm:h-6 stroke-[2.5]" />
            </button>
          </div>

          {/* Clean Viewport with Auto-Play and Full Bleed */}
          ...
          {/* Modal Footer Description - Hidden in landscape to remove bottom shadow overlay */}
